...
Object^ DotPython::Utils::ToManagedObject(ManagedPyObject^ pyObjectGuard)
{
...
DotPython::ManagedPyObject^ DotPython::Utils::ToPythonObject(Object^ managedObject)
{
    ...
    if (netDictionary != nullptr)
    {
        ...
        {
            auto pyKey = Utils::ToPythonObject(entry.Key);
            auto pyValue = Utils::ToPythonObject(entry.Value);
        ...
        type->GetGenericTypeDefinition() == System::Collections::Generic::HashSet<Object^>::typeid->GetGenericTypeDefinition())
    {
        ...
        if (hashSet != nullptr)
        {
            ...
            {
                auto pPyItem = Utils::ToPythonObject(item);
                if (pPyItem != nullptr)
                {
